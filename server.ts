...
// Global CORS Middleware to allow requests from the cross-origin OAuth workers.dev environment
...
// Custom realistic LinkedIn Search URL lead generator and campaign builder using Gemini
app.post("/api/linkedin/campaign-from-search", async (req, res) => {
  try {
    const { searchUrl, campaignId, campaignName } = req.body;

    if (!searchUrl) {
      return res.status(400).json({ error: "Missing required parameter 'searchUrl'." });
    }
  ...
  "generatedLeads": [
    ...
    const schema = {
      ...
      properties: {
        ...
        companyTitleFocus: { type: Type.STRING },
    ...
    let payload: any;
    try {
      const rawResult = await askGemini(leadGenerationPrompt, systemInstruction, schema);
      const cleanJson = rawResult.replace(/```json/gi, "").replace(/```/g, "").trim();
      payload = JSON.parse(cleanJson);
    } catch (err: any) {
      console.warn("Direct Gemini search parser failed, generating procedural fallback...", err);
      // Generate some procedurally if API fails or times out
      payload = {
    ...
    if (!targetCampaign) {
      // Create a brand new active campaign!
    ...
    // 4. Record simulation logs representing browser crawling
    db.automationLogs.push({
      timestamp: nowStr,
      level: "info",
    ...
    console.error("LinkedIn Search import error:", err);
    res.status(500).json({ error: err.message || "Scraper edge failed to parse search URL." });
  }
});
